...
public class NodeLastStep extends NodeActivityDiagram {
    ...
    public NodeLastStep ( ){
    	super();
    	step = VioletActivityEditor.manager.getOWLDataFactory().getOWLIndividual(URI.create(gm.activeOntology().getURI() + "#" +  gm.getTask() + "_end_" + cont));
    	cont++;
		OWLClass ClaseInicio = VioletActivityEditor.manager.getOWLDataFactory().getOWLClass(URI.create(ActivityGraphModel.URIAmenities + "#Final_Step"));
		
		OWLClassAssertionAxiom d = VioletActivityEditor.manager.getOWLDataFactory().getOWLClassAssertionAxiom (step, ClaseInicio);
     ...
     * @param unaPropiedadDeDatos OWL class represented by the new node
     */
    public NodeLastStep (OWLIndividual paso){
    	super();
    	step = paso;
    	addPort(east = new OWLPort(this));
        addPort(west = new OWLPort(this));
     ...
     * @return la figura
     */
    public FigLastStep getFigDataProperty(){
    	return figura;
    }
     ...
     */
    public FigLastStep makePresentation(Layer lay) {
    	
    	if (step != null){
	    	figura = new FigLastStep(step);	    	
